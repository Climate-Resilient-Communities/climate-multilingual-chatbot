...
# Import SQLite-based analytics
try:
    from dashboard.database.query_logger import get_analytics_summary
except ImportError:
    print("Warning: SQLite analytics not available. Using fallback methods.")
...
app = FastAPI(
    title="Climate Chatbot Admin API", 
    version="1.0.0",
    docs_url=None,  # Disable docs in production to save resources
    redoc_url=None  # Disable redoc in production to save resources
)
...
# Simple cache to reduce database queries
_cache = {}
...
async def get_analytics(password: str = Query(..., description="Admin password")):
    ...
    # Add comprehensive analytics from SQLite database
    try:
        # Get analytics from persistent SQLite storage
        if get_analytics_summary:
            sqlite_analytics = get_analytics_summary()
            total_interactions = sqlite_analytics.get('total_interactions', 0)
            classifications = sqlite_analytics.get('classifications', {})
            recent_queries = sqlite_analytics.get('recent_queries', {})
            print(f"Using SQLite analytics: {total_interactions} total interactions")
            
            # Override analytics_data with SQLite data for comprehensive dashboard
            analytics_data["query_analytics"] = {
                "total_interactions": total_interactions,
                "on_topic": classifications.get('on_topic', 0),
                "off_topic": classifications.get('off_topic', 0), 
                "harmful": classifications.get('harmful', 0),
                "recent_on_topic": recent_queries.get('on_topic', [])[:5],
                "recent_off_topic": recent_queries.get('off_topic', [])[:5],
                "recent_harmful": recent_queries.get('harmful', [])[:5],
                "data_source": sqlite_analytics.get('data_source', 'sqlite')
            }
            
            # Calculate interaction breakdown (maintain backward compatibility with frontend)
            if total_interactions > 0:
                analytics_data["interaction_breakdown"] = {
                    "climate_response": classifications.get('on_topic', 0),
                    "vector_search": total_interactions,  # All queries use vector search
                    "translation": max(1, int(total_interactions * 0.15)),  # Estimate 15% non-English
                    "on-topic": classifications.get('on_topic', 0),
                    "off-topic": classifications.get('off_topic', 0),
                    "harmful": classifications.get('harmful', 0)
                }
        else:
            # Fallback to feedback count if SQLite analytics unavailable
            total_interactions = analytics_data["summary"]["total_feedback"]
            print(f"SQLite analytics unavailable, using feedback count: {total_interactions}")
            analytics_data["query_analytics"] = {
                "total_interactions": total_interactions,
                "on_topic": 0,
                "off_topic": 0,
        ...
        # Interaction breakdown is now handled above in query_analytics
        ...
        # No sample recent interactions - dashboard should use detailed_queries from database
