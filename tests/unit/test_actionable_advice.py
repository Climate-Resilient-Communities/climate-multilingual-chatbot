...
async def test_community_actionable_recommendations():
    ...
    with patch('src.models.gen_response_nova.doc_preprocessing', return_value=mock_docs):
        ...
        response1, _ = await generate_chat_response(
            "What can people in Toronto do during a heat wave?", mock_docs, mock_nova_model
        )
        ...
        response2, _ = await generate_chat_response(
            "What can people in Toronto do during a heat wave?", mock_docs, mock_nova_model
        )
