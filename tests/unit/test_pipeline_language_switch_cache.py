...
@pytest.mark.asyncio
async def test_english_selected_spanish_query_still_answers(monkeypatch):
    """When English is selected but a Spanish query is sent, the pipeline no
    longer blocks with a canned mismatch message — it translates the query and
    ...
    class FakeGenerator:
        async def generate_response(self, **kwargs):
            return "Climate change refers to long-term shifts in temperatures.", []

    async def _translate(text, s=None, t=None):
        return text
    ...
    # English selected, Spanish query → answered, not blocked
    FakeCache.store = {}
    ...
    # A real answer is returned (not the canned "Whoops" mismatch message)
    assert "Climate change" in res["response"]
    assert res.get("retrieval_source") != "canned"
    assert res.get("fallback_reason") != "language_mismatch_canned"
