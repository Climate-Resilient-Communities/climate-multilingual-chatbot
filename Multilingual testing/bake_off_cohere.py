...
# --- UPDATED SOURCE TEXT AND REFERENCE TRANSLATION ---
translation_OPENAI_df = pd.read_csv('translated_OPENAI.csv')

TEST_PHRASES = translation_OPENAI_df['en_source_text'].tolist()

REFERENCE_TRANSLATIONS = {}
for lang_code in list(LANGUAGES.values()):
  REFERENCE_TRANSLATIONS[lang_code] = translation_OPENAI_df["{lang_code}_translated_OPENAI".format(lang_code = lang_code)].tolist()

# --- OLD SOURCE TEXT AND REFERENCE TRANSLATION ---
"""
TEST_PHRASES = [
...
# Initialize the Boto3 client for Bedrock Runtime (simplified version)
# try:
...
# Initialize Cohere client for Command and Aya models
try:
  cohere_client = cohere.ClientV2(api_key = "20FGf5Gegc8zvNsMiSw9xAyumBnPIaBLBJyq2eIg")
except Exception as e:
...
def get_input_tokens(cohere_response):
    input_tokens = cohere_response['meta']['tokens']['input_tokens']
    return input_tokens

def get_output_tokens(cohere_response):
    output_tokens = cohere_response['meta']['tokens']['output_tokens']
...
Tone: Informative"""
    ...
    user_prompt = f"""English text to translate: "{text}"
Translation:"""
    ...
    elif "aya" in model_id or "command" in model_id:
      # Cohere use chat/messages API format
      request_body = {
        "messages": [
          {
          "role": "system",
        ...
        # Titan models use the older format
        request_body = {
            "inputText": prompt,
    ...
    #           if "nova" in model_id:
    #               # Nova models return content in a different structure
    #               translation = response_body['output']['message']['content'][0]['text'].strip()
    #           else:
    #               # Titan models have results array
    ...
    if "aya" in model_id or "command" in model_id:
      for attempt in range(max_retries):
          try:
            ...
            translation = response.message.content[0].text
            response_time = time.time() - start_time
...
def generate_report(results_df, timestamp):
    ...
    report = f"""# Multilingual LLMs Translation Model Bake-off Report
...
def main():
    ...
    print("🚀 Starting Cohere Translation Model Bake-off\n")
            ...
            for lang_name, lang_code in LANGUAGES.items():
                for i, phrase in enumerate(TEST_PHRASES):
                    ...
                    if translation:
                        ...
                        bleu_score = bleu_result['bleu'] if bleu_result and 'bleu' in bleu_result and bleu_result['bleu'] is not None else 0.0
                        chrf_result = chrf.compute(predictions=[translation], references=[reference])
                        chrf_score = chrf_result['score'] if chrf_result and 'score' in chrf_result and chrf_result['score'] is not None else 0.0
    ...
    """
                        ...
                        remaining_updates = (len(LANGUAGES) - lang_values.index(lang_code)) * len(TEST_PHRASES) - i - 1
